...
export default async function InstructorStudentsPage() {
  ...
  .from("enrollments")
  .select("id, course_id, user_id, status, payment_status, payment_plan, start_date")
  ...
  const { data: profiles } = await supabaseAdmin
    .from("profiles")
    .select("user_id, first_name, last_name, email, phone, country, student_id")
    .in("user_id", enrollmentUserIds.length > 0 ? enrollmentUserIds : [""])
 ...
 const enrichedEnrollments = (enrollments ?? []).map((e: any) => {
  const profile = profiles?.find((p) => p.user_id === e.user_id) ?? null
